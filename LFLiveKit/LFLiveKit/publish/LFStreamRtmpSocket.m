        ...
        if (!_self.isSending && _self.buffer.list.count > 0) {
            ...
            if ([frame isKindOfClass:[LFVideoFrame class]]) {
                ...
                if (!_self.sendAudioHead) {
                    ...
                    switch(self->_stream.audioConfiguration.audioCodecType){
                        case LFLiveAudioCodecType_PCM:
                            // bugfix: g0 can't resolve this 3 bytes. So don't send AudioHeader
                            // [_self sendAudioHeader:(LFAudioFrame *)frame];
                            break;
                        case LFLiveAudioCodecType_AMR:
                            [_self sendAudioHeaderAMR:(LFAudioFrame *)frame];
                            break;
                        case LFLiveAudioCodecType_AAC:
                            //[_self sendAudioHeaderAAC:(LFAudioFrame *)frame];
                            break;
                        default:
                            [_self sendAudioHeader:(LFAudioFrame *)frame];
                            break;
                    ...
                    switch(self->_stream.audioConfiguration.audioCodecType){
                        case LFLiveAudioCodecType_PCM:
                            [_self sendAudio:(LFAudioFrame *)frame];
                            break;
                        case LFLiveAudioCodecType_AMR:
                            [_self sendAudioAMR:(LFAudioFrame *)frame];
                            break;
                        case LFLiveAudioCodecType_AAC:
                            //[_self sendAudioAAC:(LFAudioFrame *)frame];
                            break;
                        default:
                            [_self sendAudio:(LFAudioFrame *)frame];
                            break;
...
- (void)sendAudioHeader:(LFAudioFrame *)audioFrame {
    ...
    body[0] = 0x32;
//    body[1] = 0x00;
    memcpy(&body[1], audioFrame.audioInfo.bytes, audioFrame.audioInfo.length);          /*spec_buf是AAC sequence header数据*/
...
- (void)sendAudio:(LFFrame *)frame {
    ...
    NSInteger rtmpLength = frame.data.length + 1;    /*spec data长度,一般是2*/
    unsigned char *body = (unsigned char *)malloc(rtmpLength);
    memset(body, 0, rtmpLength);
    ...
    body[0] = 0x32;
//    body[1] = 0x01;
    memcpy(&body[1], frame.data.bytes, frame.data.length);
    [self sendPacket:RTMP_PACKET_TYPE_AUDIO data:body size:rtmpLength nTimestamp:frame.timestamp];
    free(body);
}

- (void)sendAudioHeaderAMR:(LFAudioFrame *)audioFrame {
...
- (void)sendAudioAMR:(LFFrame *)frame {
...
void rtmp_log_cb(int level, const char *fmt, va_list va)
{
    NSLogv([NSString stringWithCString:fmt encoding:NSUTF8StringEncoding], va);
}
