...
//        startLiveButton.addTarget(self, action: #selector(didTappedStartLiveButton(_:)), for: .touchUpInside)
        ...
        stream.url = "rtsp://52.81.45.95/live/testaac"
        stream.sendProtcol = .RTSP
    ...
    func liveSession(_ session: LFLiveSession?, debugInfo: LFLiveDebug?) {
        print("LFLiveSession debugInfo uploadSpeed: " + self.formatedSpeed(bytes: Float(debugInfo?.currentBandwidth ?? 0), elapsedMilli: Float(debugInfo?.elapsedMilli ?? 0) ))
    }
    
    func liveSession(_ session: LFLiveSession?, errorCode: LFLiveSocketErrorCode) {
        switch errorCode {
        case .preView:
            print("LiveSession preView")
        case .getStreamInfo:
            print("LiveSession getStreamInfo")
        case .connectSocket:
            print("LiveSession connectSocket")
        case .verification:
            print("LiveSession verification")
        case .reConnectTimeOut:
            print("LiveSession reConnectTimeOut")
        }
    }
    ...
    func liveSession(_ session: LFLiveSession?, liveStateDidChange state: LFLiveState) {
        switch state {
        case .ready:
            print("LFLiveSession liveStateDidChange ready")
        case .pending:
            print("LFLiveSession liveStateDidChange pending")
        case .start:
            print("LFLiveSession liveStateDidChange start")
        case .stop:
            print("LFLiveSession liveStateDidChange stop")
        case .error:
            print("LFLiveSession liveStateDidChange error")
        case .refresh:
            print("LFLiveSession liveStateDidChange refresh")
        }
    }
    
    func formatedSpeed(bytes : Float , elapsedMilli : Float) -> String {
        guard elapsedMilli > 0 else {
            return "N/A"
        }
        ...
        let bytes_per_sec : Float = bytes * 1000 /  elapsedMilli;
        
        if (bytes_per_sec >= 1000 * 1000) {
            return "\(bytes_per_sec / 1000 / 1000) MB/s";
        } else if (bytes_per_sec >= 1000) {
            return "\(bytes_per_sec / 1000 ) KB/s";
        } else {
...
//    }
    ...
    @objc func didTappedStartLiveButton(_ button: UIButton) -> Void {
        ...
        if (startLiveButton.isSelected) {
            ...
            let stream = LFLiveStreamInfo()
            stream.url = "rtmp://52.81.66.238/live/720c0ec2_1559195473_audio"
            session.startLive(stream)
        } else {
    ...
    var session: LFLiveSession = {
        let videoC = LFLiveVideoConfiguration()
        let session = LFLiveSession(audioConfiguration: LFLiveAudioConfiguration.aac(), videoConfiguration: videoC , captureType: LFLiveCaptureTypeMask.captureMaskAudio)
    ...
    var startLiveButton: UIButton = {
        ...
        startLiveButton.backgroundColor = UIColor.orange
