...
@property(nonatomic, readonly) int64_t tcpSpeed;
...
typedef NS_ENUM(NSInteger , IJKMPRecordState) {
    IJKMPRecordStart,
    IJKMPRecordEnd,
    IJKMPRecordStartError,
    IJKMPRecordEndError,
};

typedef NS_ENUM(NSInteger , IJKMPConnectState) {
    IJKMPConnectUrlNotFound
};

@protocol IJKFFMoviePlayerControllerProtocol <NSObject>
- (void)recordStateChange:(IJKFFMoviePlayerController *)sender  RecordStater:(IJKMPRecordState)recordState error:(int)errorCode VideoPath:(NSString *)videoPath;
- (void)downloadSpeed:(IJKFFMoviePlayerController *)sender DownloadSpeed:(NSInteger)downloadSpeed;
- (void)screenshot:(IJKFFMoviePlayerController *)sender ScreenImage:(UIImage *)image;
- (void)connectStateChange:(IJKFFMoviePlayerController *)sender ConnectState:(IJKMPConnectState) state  Msg:(NSString *) msg;
@end
