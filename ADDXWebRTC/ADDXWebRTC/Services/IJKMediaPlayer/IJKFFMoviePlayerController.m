...
@synthesize audioEnable = _audioEnable;
@synthesize tcpSpeed = _tcpSpeed;
...
#warning “change back to 100 when howling suppression is enabled”
    self.playbackVolume = audioEnable ? 75 : 0 ;
}

-(Boolean)audioEnable {
    return self.playbackVolume > 50;
}
...
- (void)dealloc
{
    ...
    //bugfix: ios maybe crash because GLView not free
    [self shutdown];
}
...
- (void)startRecord {
    ...
    NSString * videoPath = [cachePath stringByAppendingPathComponent:@"recoredCache.mp4"];
    if ([[NSFileManager defaultManager] fileExistsAtPath:videoPath]) {
        [[NSFileManager defaultManager] removeItemAtPath:videoPath error:nil];
    }
    fileVideoPath = videoPath;
...
- (void)shutdown
{
    if (!_mediaPlayer)
        return;
    //bugfix: ios maybe crash because _glView not free
...
- (int64_t)tcpSpeed {
    if (!_mediaPlayer)
        return 0;
    _tcpSpeed = ijkmp_get_property_int64(_mediaPlayer, FFP_PROP_INT64_TCP_SPEED, 0);
    return _tcpSpeed;
}
...
- (void)setPlaybackVolume:(float)volume
{
    if (!_mediaPlayer)
        return;
    BOOL muted = fabs(volume) < 1e-6;
    ijkmp_set_playback_muted(_mediaPlayer, muted);
...
- (void)postEvent: (IJKFFMoviePlayerMessage *)msg
{
    ...
    switch (avmsg->what) {
        case FFP_MSG_FLUSH:
            break;
        case FFP_MSG_ERROR_RTMP_URL_NOT_FOUND:
            NSLog(@"FFP_MSG_ERROR_RTMP_URL_NOT_FOUND: %d\n", avmsg->arg1);
            if (self.delegate != nil && [self.delegate respondsToSelector:@selector(connectStateChange:ConnectState:Msg:)]) {
                [self.delegate connectStateChange:self ConnectState:IJKMPConnectUrlNotFound Msg:@"IJKMPConnectUrlNotFound"];
            }
            break;
        case FFP_MSG_ERROR: {
        ...
        case FFP_MSG_BUFFERING_BYTES_UPDATE:
            if(self.delegate != nil && [self.delegate respondsToSelector:@selector(downloadSpeed:DownloadSpeed:)]){
                [self.delegate downloadSpeed:self DownloadSpeed:avmsg->arg1];
            }
        ...
        case FFP_PROP_VIDEO_RECORD_ERROR : {
            if (self.delegate != nil && [self.delegate respondsToSelector:@selector(recordStateChange:RecordStater:error:VideoPath:)]) {
                [self.delegate recordStateChange:self RecordStater:IJKMPRecordEndError error:0 VideoPath:fileVideoPath];
                
            }
            if ([[NSFileManager defaultManager] fileExistsAtPath:fileVideoPath]) {
                [[NSFileManager defaultManager] removeItemAtPath:fileVideoPath error:nil];
            }
            break;
            
        }
        case FFP_PROP_VIDEO_RECORD_START : {
            if (self.delegate != nil && [self.delegate respondsToSelector:@selector(recordStateChange:RecordStater:error:VideoPath:)]) {
                [self.delegate recordStateChange:self RecordStater:IJKMPRecordStart error:0 VideoPath:fileVideoPath];
            }
            break;
        }
            
        case FFP_PROP_VIDEO_RECORD_END : {
            if (self.delegate != nil && [self.delegate respondsToSelector:@selector(recordStateChange:RecordStater:error:VideoPath:)]) {
                [self.delegate recordStateChange:self RecordStater:IJKMPRecordEnd error:0 VideoPath:fileVideoPath];
