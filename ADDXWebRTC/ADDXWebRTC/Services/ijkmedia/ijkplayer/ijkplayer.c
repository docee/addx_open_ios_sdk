...
void ijkmp_set_playback_muted(IjkMediaPlayer *mp, bool muted) {
    assert(mp);
    MPTRACE("%s(%f)\n", __func__, muted);
    pthread_mutex_lock(&mp->mutex);
    ffp_set_playback_muted(mp->ffplayer, muted);
    pthread_mutex_unlock(&mp->mutex);
    MPTRACE("%s()=void\n", __func__);
}
...
int ijkmp_start_recording(IjkMediaPlayer *mp,const char *file_name)
{
    assert(mp);
...
int ijkmp_stop_recording(IjkMediaPlayer *mp)
{
    ...
    if (retval == 0) {
        ffp_notify_msg2(mp->ffplayer ,FFP_PROP_VIDEO_RECORD_END ,0);
    }else {
        ffp_notify_msg2(mp->ffplayer ,FFP_PROP_VIDEO_RECORD_ERROR ,0);
    }
    return retval;
}
