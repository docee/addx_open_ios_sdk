...
#define FFP_RTMP_URL_NOT_FOUND_CODE -10181
        ...
        if (frame_queue_nb_remaining(&is->pictq) == 0) {
            ...
            if (time < is->frame_timer + delay) {
                *remaining_time = FFMIN(is->frame_timer + delay - time, *remaining_time);
                //av_log(NULL, AV_LOG_ERROR,"pictq o1 %f", vp->pts);
                goto display;
            }
            ...
            SDL_UnlockMutex(ffp->is->play_mutex);
            av_log(NULL, AV_LOG_ERROR,"pictq o2 %f", vp->pts);
        }
display:
        /* display picture */
...
static int ffplay_video_thread(void *arg)
{
    ...
    for (;;) {
        ...
        while (ret >= 0) {
            ...
            //av_log(NULL, AV_LOG_ERROR, "dur: %f picq in %f\n", duration, pts);
...
{
    av_log(NULL, AV_LOG_FATAL, "read_thread() created\n");
    ...
    av_log(NULL, AV_LOG_FATAL, "avformat_alloc_context()\n");
    ...
    av_log(NULL, AV_LOG_WARNING, "chen:avformat_open_input before\n");
    //fortest
    //char* testUrl = "rtmp://mobliestream.c3tv.com:554/live/goodtv.sdp";
    //err = avformat_open_input(&ic, testUrl, is->iformat, &ffp->format_opts);
    err = avformat_open_input(&ic, is->filename, is->iformat, &ffp->format_opts);
    av_log(NULL, AV_LOG_WARNING, "chen:avformat_open_input after, filename=%s\n",
    is->filename);
    if (err < 0) {
        if (av_stristart(is->filename, "rtmp", NULL) ||
            av_stristart(is->filename, "rtsp", NULL)) {
            last_error = FFP_RTMP_URL_NOT_FOUND_CODE; //random
            char errbuf[1024] = {0};
            av_strerror(err, errbuf, 1024);
            av_log(NULL, AV_LOG_ERROR, "chen:avformat_open_input rtmp err=%s\n", errbuf);
        }
        print_error(is->filename, err);
        ret = -1;
        goto fail;
    }
    if (av_stristart(is->filename, "rtsp", NULL) && ic->nb_streams < 2){
        last_error = FFP_RTMP_URL_NOT_FOUND_CODE; //random
        av_log(NULL, AV_LOG_ERROR, "chen:avformat_open_input rtmp err=%d\n", err);
        print_error(is->filename, err);
        ret = -1;
        goto fail;
    }
    ...
    //av_log(NULL, AV_LOG_FATAL, "av_format_inject_global_side_data() before \n");
    av_format_inject_global_side_data(ic);

    //av_log(NULL, AV_LOG_FATAL, "av_format_inject_global_side_data() after\n");
    //
    //AVDictionary **opts;
    //int orig_nb_streams;
    ...
    if (ffp->find_stream_info) {
        //av_log(NULL, AV_LOG_WARNING, "chen:setup_find_stream_info_opts before\n");
        AVDictionary **opts = setup_find_stream_info_opts(ic, ffp->codec_opts);
        //av_log(NULL, AV_LOG_WARNING, "chen:setup_find_stream_info_opts after\n");
        ...
        do {
            ...
            av_log(NULL, AV_LOG_WARNING, "chen:avformat_find_stream_info before %d\n",i);
            err = avformat_find_stream_info(ic, opts);
            av_log(NULL, AV_LOG_WARNING, "chen:avformat_find_stream_info after %d\n",i);
        } while(0);
    ...
    for (;;) {
        ...
        if (!ffp->is_first && pkt->pts == pkt->dts) { // 获取开始录制前dts等于pts最后的值，用于
            if (pkt->stream_index >= 0 && pkt->stream_index < is->ic->nb_streams &&
                is->ic->streams[pkt->stream_index]->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
                ffp->audio_start_pts = pkt->pts;
                ffp->audio_start_dts = pkt->dts;
            }
        }
        if (pkt->stream_index >= 0 && pkt->stream_index < is->ic->nb_streams &&
            is->ic->streams[pkt->stream_index]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
            if (!ffp->is_first) {
                ffp->video_start_pts = pkt->pts;
                ffp->video_start_dts = pkt->dts;
                //                av_log(ffp, AV_LOG_ERROR, "set start video start_pts: %lld +++++ start_dts: %lld\n", ffp->start_pts, ffp->start_dts);
            }
        }
...
//      of the seek_pos/seek_rel variables
        ...
        //av_log(NULL, AV_LOG_WARNING, "chen:av_read_frame before\n");
        ret = av_read_frame(ic, pkt);

        //av_log(NULL, AV_LOG_WARNING, "chen:av_read_frame after\n");
        if (ret < 0) {
            int pb_eof = 0;
            int pb_error = 0;
        ...
        if (pkt->flags & AV_PKT_FLAG_DISCONTINUITY) {
            if (is->audio_stream >= 0) {
                //av_log(NULL, AV_LOG_WARNING, "chen:audio discont\n");
                packet_queue_put(&is->audioq, &flush_pkt);
            }
            if (is->subtitle_stream >= 0) {
                //av_log(NULL, AV_LOG_WARNING, "chen:sub discount\n");
                packet_queue_put(&is->subtitleq, &flush_pkt);
            }
            if (is->video_stream >= 0) {
                //av_log(NULL, AV_LOG_WARNING, "chen:video discount\n");
                packet_queue_put(&is->videoq, &flush_pkt);
            }
        }
        ...
        if ((av_stristart(ffp->input_filename, "rtsp", NULL) ||
              av_stristart(ffp->input_filename, "rtsps", NULL)) &&
            pkt->stream_index == is->video_stream) {
            //av_log(NULL, AV_LOG_INFO, "## pts %llu\n", pkt->pts);
...
#pragma  mark - 录制插入
        if (ffp->is_record) { // 可以录制时，写入文件
            if (pkt->flags & AV_PKT_FLAG_KEY) {
                ffp->is_first_keyframe_recorded = true;
            }
            if (ffp->is_first_keyframe_recorded) {
                if (0 != ffp_record_file(ffp, pkt)) {
                    av_log(NULL, AV_LOG_ERROR,"ffp_stop_recording_l\n", pkt->pts, pkt->dts);
                    ffp->record_error = 1;
                    ffp->is_record = 0;
                    ffp_stop_record(ffp);
                    ffp_notify_msg2(ffp ,FFP_PROP_VIDEO_RECORD_ERROR ,-1);
                }
            }
        }

        if (pkt->stream_index == is->audio_stream && pkt_in_play_range) {
            //av_log(NULL, AV_LOG_WARNING, "chen:queue audio %lld\n", pkt->pts);
            packet_queue_put(&is->audioq, pkt);
        } else if (pkt->stream_index == is->video_stream && pkt_in_play_range
                   && !(is->video_st && (is->video_st->disposition & AV_DISPOSITION_ATTACHED_PIC))) {
            //av_log(NULL, AV_LOG_WARNING, "chen:queue video %lld\n", pkt->pts);
            packet_queue_put(&is->videoq, pkt);

            /*
            if (count < 30) {
                av_log(NULL, AV_LOG_INFO, "du: write client.raw before\n");

                char *filename[32];
                sprintf(filename, "/mnt/sdcard/DCIM/client_%d.raw", count);

                FILE *fid = fopen(filename, "w+");

                if (fid == NULL) {
                    av_log(NULL, AV_LOG_INFO, "client.raw is null: %d\n", pkt->size);
                } else {
                    fwrite(pkt->data, 1, pkt->size, fid);
                    fclose(fid);
                }
                count++;
                av_log(NULL, AV_LOG_INFO, "du: write client.raw after\n");
            }*/
            //av_log(NULL, AV_LOG_ERROR, "vq in %lld\n", pkt->pts);

        } else if (pkt->stream_index == is->subtitle_stream && pkt_in_play_range) {
            packet_queue_put(&is->subtitleq, pkt);
        } else {
            //av_log(NULL, AV_LOG_WARNING, "chen:queue unref %lld\n", pkt->pts);
            av_packet_unref(pkt);
        }
 ...
 fail:
    ...
    if (!ffp->prepared || !is->abort_request) {
        ...
        if(last_error == FFP_RTMP_URL_NOT_FOUND_CODE) {
            ffp_notify_msg2(ffp, FFP_MSG_ERROR_RTMP_URL_NOT_FOUND, last_error);
        }else{
...
static int video_refresh_thread(void *arg)
{
    ...
    av_log(NULL, AV_LOG_ERROR, "video_refresh_thread() created\n");
    FFPlayer *ffp = arg;
    VideoState *is = ffp->is;
...
long ffp_get_current_position_l(FFPlayer *ffp)
{
    ...
    // Bug: fix rtsp play sdcard record， timestamp issue.
    if ((av_stristart(ffp->input_filename, "rtsp", NULL) ||
         av_stristart(ffp->input_filename, "rtsps", NULL))) {
        //av_log(NULL, AV_LOG_INFO, "## currentVideoPts = %ld\n", currentVideoPts);
        return (long)(currentVideoPts / 90.0 * 1000);
    }
...
void ffp_set_playback_muted(FFPlayer *ffp, bool muted) {
    if (!ffp || !ffp->is) {
        return;
    }
    VideoState *is = ffp->is;
...
int ffp_start_record(FFPlayer *ffp, const char *file_name)
{
    assert(ffp);
    VideoState *is = ffp->is;
    av_log(ffp, AV_LOG_ERROR, "ffp_start_recording_l");

    ffp->m_ofmt_ctx = NULL;
    ffp->m_ofmt = NULL;
    ffp->is_record = 0;
    ffp->is_first_keyframe_recorded = false;
    ffp->record_error = 0;
    ...
    for (int i = 0; i < is->ic->nb_streams; i++) {
        ...
        if(out_stream->codec->codec_id == AV_CODEC_ID_HEVC){
    ...
    ffp_notify_msg2(ffp ,FFP_PROP_VIDEO_RECORD_START ,0);
    return 0;
end:
    ffp->record_error = 1;
    ffp_notify_msg2(ffp ,FFP_PROP_VIDEO_RECORD_ERROR ,-1);
    ...
    if (ffp->is_record) {
        ...
        if (0 == av_packet_ref(pkt, packet)) {
            ...
            if (!ffp->is_first) { // 录制的第一帧，时间从0开始
                ...
                if (pkt->stream_index >= 0 && pkt->stream_index < ffp->m_ofmt_ctx->nb_streams) {
                    if (ffp->m_ofmt_ctx->streams[pkt->stream_index]->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
                        //bugfix: rtsp audio stream and video stream use diff pts/dts, so can't use same start_pts/start_dts
                        pkt->pts = llabs(pkt->pts - ffp->audio_start_pts);
                        pkt->dts = llabs(pkt->dts - ffp->audio_start_dts);
                        
                        //av_log(ffp, AV_LOG_DEBUG, "### audio pts: %llu dts: %llu ffp->audio_start_pts: %llu ffp->audio_start_dts: %llu\n",
                        //       pkt->pts, pkt->dts, ffp->audio_start_pts, ffp->audio_start_dts);
                        
                    } else if (ffp->m_ofmt_ctx->streams[pkt->stream_index]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO){
                        pkt->pts = pkt->dts = llabs(pkt->dts - ffp->video_start_dts);
                        
                        //av_log(ffp, AV_LOG_DEBUG, "### video pts: %llu dts: %llu ffp->video_start_pts: %llu ffp->video_start_dts: %llu\n",
                        //       pkt->pts, pkt->dts, ffp->video_start_pts, ffp->video_start_dts);
                    }
                } else {
                    av_log(ffp, AV_LOG_ERROR, "Invalid stream_index: %d\n", pkt->stream_index);
                    return -1;
                }
                /*
                if (pkt->stream_index == AVMEDIA_TYPE_AUDIO) {
                    pkt->pts = pkt->pts - ffp->start_pts > 0 ? pkt->pts - ffp->start_pts : 0;
                    pkt->dts = pkt->dts - ffp->start_dts > 0 ? pkt->dts - ffp->start_dts : 0;
                }
                else if (pkt->stream_index == AVMEDIA_TYPE_VIDEO) {
                    pkt->pts = pkt->dts = pkt->dts - ffp->start_dts > 0 ? pkt->dts - ffp->start_dts : 0;
                }
                 */
            }
