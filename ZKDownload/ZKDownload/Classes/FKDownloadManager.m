...
- (void)setupSession {
    ...
    if (self.session) {
        ...
        if (self.configure.isBackgroudExecute) {
            FKLog(@"配置 NSURLSession self.configure.isBackgroudExecute")
...
- (nullable FKTask *)acquire:(NSString *)url {
    ...
    FKTask *task = [self.taskHub taskWithIdentifier:[url urlComment].identifier];
    if (task) {
        return task;
    }
    
    task = [self.taskHub taskWithIdentifier:[url urlComment].SHA256];
    if (task) {
        return task;
    }
...
- (FKTask *)createPreserveTask:(NSString *)url number:(NSUInteger)number{
    ...
    FKTask *existTask = [self.taskHub taskWithIdentifier:[url urlComment].identifier];
    if (existTask) {
        return existTask;
    }
...
- (void)startWithAll {
    NSArray<FKTask *> * allTask = self.taskHub.allTask ;
    [self startWithTasks:allTask] ;
}

- (void)startWithTasks:(NSArray<FKTask *> *) allTask {
    self.isControlTasks = YES;
    __weak typeof(self) weak = self;
    __block BOOL isDone = true;
    [allTask forEach:^(FKTask *task, NSUInteger idx) {
        [weak start:task.url];
    ...
    if (isDone) {
        if (self.downloadEndBlock) {
            self.downloadEndBlock();
        }
    }
...
- (void)startWithTag:(NSString *)tag {
    NSArray<FKTask *> * allTask = [self.taskHub taskForTag:tag] ;
    [self startWithTasks:allTask] ;
}
...
- (void)cancelEndTasks {
    ...
    NSArray<FKTask *> * errorTask = [[FKDownloadManager manager] filterTaskWithStatus:TaskStatusUnknowError];
    NSArray<FKTask *> * execingTask = [[FKDownloadManager manager] filterTaskWithStatus:TaskStatusExecuting];
    NSArray<FKTask *> * cancelTask = [[FKDownloadManager manager] filterTaskWithStatus:TaskStatusCancelld];
    NSArray<FKTask *> * finishTask = [[FKDownloadManager manager] filterTaskWithStatus:TaskStatusFinish];
    NSArray<FKTask *> * suspendTask = [[FKDownloadManager manager] filterTaskWithStatus:TaskStatusSuspend];
    [array addObjectsFromArray:errorTask];
    [array addObjectsFromArray:execingTask];
    [array addObjectsFromArray:cancelTask];
    [array addObjectsFromArray:finishTask];
    [array addObjectsFromArray:suspendTask];
    self.isControlTasks = YES;
    [array forEach:^(FKTask *task, NSUInteger idx) {
        [self cancel:task.url];
    }];
    self.isControlTasks = NO;
}
...
- (void)updateQueueBlock:(FKTask *) task {
    if (self.queueUpdateBlock) {
        NSUInteger prepareCount = [[FKDownloadManager manager] filterTaskWithStatus:TaskStatusPrepare].count;
        NSUInteger idleCount = [[FKDownloadManager manager] filterTaskWithStatus:TaskStatusIdle].count;

        
        NSUInteger errorCount = [[FKDownloadManager manager] filterTaskWithStatus:TaskStatusUnknowError].count;
        NSArray<FKTask *> * execing = [[FKDownloadManager manager] filterTaskWithStatus:TaskStatusExecuting];

        NSUInteger cancelCount = [[FKDownloadManager manager] filterTaskWithStatus:TaskStatusCancelld].count;
        NSUInteger finishCount = [[FKDownloadManager manager] filterTaskWithStatus:TaskStatusFinish].count;
        
        NSUInteger total = self.taskHub.allTask.count - cancelCount;
        NSUInteger excueIndex = finishCount + execing.count ;
        if (![execing containsObject:task]) {
            NSMutableArray * array = [[NSMutableArray alloc] initWithArray:execing] ;
            [array addObject:task];
            execing = array ;
        }
        
        self.queueUpdateBlock(execing , (int)total , (int)(prepareCount + idleCount) , (int)excueIndex , (int)errorCount);
    }
}
...
/**
 更新任务到下一个执行
 */
- (void)moveTaskFirst:(NSString *) url {
    checkURL(url);
    ...
    FKTask *nextTask = [[[FKDownloadManager manager] filterTaskWithStatus:TaskStatusIdle] sortedArrayUsingDescriptors:@[[NSSortDescriptor sortDescriptorWithKey:@"number" ascending:YES]]].firstObject;
    if ([task.identifier isEqualToString:nextTask.identifier]){
        NSLog(@"current next is %@",url);
        return;
    }

    int firstNum = [[task valueForKey:@"number"] intValue];
    [task setValue:@(firstNum-1) forKey:@"number"];
}
