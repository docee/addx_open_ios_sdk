...
- (void)URLSessionDidFinishEventsForBackgroundURLSession:(NSURLSession *)session {
    FKLog(@"FKDownloadExecutor URLSessionDidFinishEventsForBackgroundURLSession")
...
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didCompleteWithError:(NSError *)error {
    FKLog(@"FKDownloadExecutor didCompleteWithError")
        ...
        if (error.code == NSURLErrorCancelled && error.userInfo[NSURLSessionDownloadTaskResumeData]) {
            ...
            NSError *error = [NSError errorWithDomain:NSURLErrorDomain
                                             ...
                                             userInfo:@{NSFilePathErrorKey: url.absoluteString,
                                                        NSLocalizedDescriptionKey: [NSString stringWithFormat:@"HTTP Status Code: %d", (int)1000]}];
...
- (void)URLSession:(NSURLSession *)session downloadTask:(NSURLSessionDownloadTask *)downloadTask didFinishDownloadingToURL:(NSURL *)location {
    FKLog(@"FKDownloadExecutor didFinishDownloadingToURL")
