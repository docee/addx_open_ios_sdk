...
@property (nonatomic , assign) Boolean isListen;
...
- (void)settingInfo:(NSDictionary *)info {
    ...
    if ([info.allKeys containsObject:FKTaskInfoMediaType]) {
        id mediaType = info[FKTaskInfoMediaType];
        if ([mediaType isKindOfClass:[NSNumber class]]) {
            self.mediaType = [mediaType integerValue];
        }
    }
    
    if ([info.allKeys containsObject:FKTaskInfoUserInfo]) {
        id userinfo = info[FKTaskInfoUserInfo];
        if ([userinfo isKindOfClass:[NSDictionary class]]) {
            self.userInfo = userinfo;
        }
    }
    if ([info.allKeys containsObject:FKTaskInfoDescribe]) {
        id describe = info[FKTaskInfoDescribe];
        if ([describe isKindOfClass:[NSString class]]) {
...
- (void)execute {
    FKLog(@"执行: %@", self)
    [self.manager updateQueueBlock:self];
    ...
    if ((!self.manager.configure.isAllowCellular && (self.manager.reachability.currentReachabilityStatus == ReachableViaWWAN))) {
        
        self.error = [NSError errorWithDomain:NSURLErrorDomain
                                         code:NSURLErrorNotConnectedToInternet
                                     userInfo:@{NSFilePathErrorKey: self.url,
                                                NSLocalizedDescriptionKey: [NSString stringWithFormat:@"Network Unavailable isAllowCellular"]}];
...
- (void)sendWillCancelldInfo {
    if ([self.delegate respondsToSelector:@selector(downloader:willCanceldTask:)]) {
        __weak typeof(self) weak = self;
        dispatch_async(dispatch_get_main_queue(), ^{
            [weak.delegate downloader:self.manager willCanceldTask:self];
        });
    }
    if (self.statusBlock) {
        __weak typeof(self) weak = self;
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([weak isKindOfClass:[NSNull class]] || weak == nil) {
                return ;
            }
            weak.statusBlock(weak);
        });
...
- (void)sendProgressInfo {
    ...
    if ([self.delegate respondsToSelector:@selector(downloader:progressingTask:)]) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (weak != nil) {
                [weak.delegate downloader:weak.manager progressingTask:weak];
            }
        });
    }
...
- (NSString *)filePath {
    if (self.fileName.length) {
        ...
        NSArray * tagsArr = [self.tags allObjects];
        if (tagsArr.count > 0) {
            BOOL isDirectory = NO;
            BOOL isFileExist = NO;
            NSString *mediaTypeSavePath = [self.manager.configure.savePath stringByAppendingPathComponent:tagsArr[0]];
            isFileExist = [self.manager.fileManager fileExistsAtPath:mediaTypeSavePath isDirectory:&isDirectory];
            if (!(isFileExist && isDirectory)) {
                [self.manager.fileManager createDirectoryAtPath: mediaTypeSavePath
                                    withIntermediateDirectories:YES
                                                     attributes:nil
                                                          error:nil];
                
            }
            return [mediaTypeSavePath stringByAppendingPathComponent:fileName];
        } else {
...
- (void)setUrl:(NSString *)url {
    ...
    self.identifier = [[url urlComment] SHA256];
